...
class SNMPAgent {
    public:
        ...
        /** Direct read of an OID already registered on this agent (same data as SNMP GET, no UDP). */
        enum class SnmpRegisteredOidReadStatus : uint8_t {
            Ok,
            InvalidOidFormat,
            NotRegistered,
            ReadFailed
        };
        SnmpRegisteredOidReadStatus syncReadRegisteredOid(const std::string& oidInput,
