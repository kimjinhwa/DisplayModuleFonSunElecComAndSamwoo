...
static std::string normalizeWebOidInput(std::string raw)
{
    std::string out;
...
static const char *asnTypeName(ASN_TYPE t)
{
...
static void appendHexBytes(std::string &out, const uint8_t *p, int len, int maxBytes)
{
...
SNMPAgent::SnmpRegisteredOidReadStatus SNMPAgent::syncReadRegisteredOid(const std::string &oidInput,
                                                                        std::string &canonicalOidOut,
                                                                        std::string &asnTypeNameOut,
                                                                        std::string &valueDisplayOut)
{
    canonicalOidOut.clear();
    asnTypeNameOut.clear();
    valueDisplayOut.clear();

    const std::string norm = normalizeWebOidInput(oidInput);
    if (norm.empty())
        return SnmpRegisteredOidReadStatus::InvalidOidFormat;

    SortableOIDType key(norm);
    if (!key.valid)
        return SnmpRegisteredOidReadStatus::InvalidOidFormat;

    ValueCallback *cb = ValueCallback::findCallback(this->callbacks, &key, false);
    if (!cb)
        return SnmpRegisteredOidReadStatus::NotRegistered;

    std::shared_ptr<BER_CONTAINER> val = ValueCallback::getValueForCallback(cb);
    if (!val)
        return SnmpRegisteredOidReadStatus::ReadFailed;

    canonicalOidOut = cb->OID->string();
    asnTypeNameOut = asnTypeName(val->_type);
    ...
    case COUNTER32:
    case GAUGE32: {
        auto *iv = static_cast<IntegerType *>(val.get());
        char buf[32];
        if (val->_type == GAUGE32 || val->_type == COUNTER32)
            snprintf(buf, sizeof(buf), "%u", static_cast<unsigned>(iv->_value));
        else
            snprintf(buf, sizeof(buf), "%d", iv->_value);
        valueDisplayOut = buf;
        break;
    }
    case COUNTER64: {
        auto *c64 = static_cast<Counter64 *>(val.get());
        char buf[32];
        snprintf(buf, sizeof(buf), "%" PRIu64, static_cast<unsigned long long>(c64->_value));
        valueDisplayOut = buf;
        break;
    }
    case STRING: {
        const std::string &s = static_cast<OctetType *>(val.get())->_value;
        if (isPrintableAscii(s))
            valueDisplayOut = s;
        else
            appendHexBytes(valueDisplayOut,
    ...
    case OID:
        valueDisplayOut = static_cast<OIDType *>(val.get())->string();
        break;
    case OPAQUE: {
        auto *op = static_cast<OpaqueType *>(val.get());
        if (op->_value && op->_dataLength > 0)
            appendHexBytes(valueDisplayOut, op->_value, op->_dataLength, 256);
        else
            valueDisplayOut = "";
        break;
    }
    default:
        valueDisplayOut = "(unhandled type)";
        break;
    }

    return SnmpRegisteredOidReadStatus::Ok;
}
